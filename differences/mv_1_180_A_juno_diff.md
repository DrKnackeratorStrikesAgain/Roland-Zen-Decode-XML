# Schema differences (`MV-1-180-A` vs `Juno`)
...
| Metric | MV-1-180-A | Juno |
|--------|--------:|-----:|
| Top-level blocks (items) | 83 | 94 |
| Blocks only in first | 52 | — |
| Blocks only in second | — | 63 |
| Blocks in both | 31 | 31 |
| Shared blocks with differing `byteLength` or `sysexLength` | 5 | — |
| Shared blocks with parameter / property differences | 20 | — |
...
| `MSMP_SPLT` | Multisample Split | 4 | 6 |
| `MVProj` |  | 5544 |  |
| `MultiSample` |  | 528 |  |
| `PCMRInst` |  | 216 |  |
| `PHR` |  | 192 | 177 |
...
| Block | MV-1-180-A byteLength | Juno byteLength | MV-1-180-A sysexLength | Juno sysexLength |
|-------|----------------:|---------------:|------------------:|----------------:|
...
- **Only in second (Juno):** `VA_INIT_PHASE`
...
- Properties **only on second (Juno)**: `displayRange`
...
- Properties **only on second (Juno)**: `displayMeasurement`, `sysexValueOffset`
...
- Properties **only on second (Juno)**: `sysexOffset`
